import React, { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Icons8 } from "@/components/ui/icons8";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { apiClient } from "@/lib/apiClient";
import { toast } from "sonner";
import { ParkingSite } from "@/store/useSiteStore";

interface PrinterModalProps {
  isOpen: boolean;
  onClose: () => void;
  printer: any | null;
  sites: ParkingSite[];
}

export function PrinterModal({ isOpen, onClose, printer, sites }: PrinterModalProps) {
  const queryClient = useQueryClient();
  const [name, setName] = useState("");
  const [ip, setIp] = useState("");
  const [siteId, setSiteId] = useState("");
  const [isDefault, setIsDefault] = useState(false);
  const [printSimultaneously, setPrintSimultaneously] = useState(true);

  useEffect(() => {
    if (isOpen) {
      setName(printer?.name || "");
      setIp(printer?.ip || "");
      setSiteId(printer?.siteId || (sites.length > 0 ? sites[0].id : ""));
      setIsDefault(printer?.isDefault ?? false);
      setPrintSimultaneously(printer?.printSimultaneously ?? true);
    }
  }, [isOpen, printer, sites]);

  const mutation = useMutation({
    mutationFn: async (data: any) => {
      if (printer) {
        await apiClient.patch(`/api/printer/${printer.id}`, data);
      } else {
        await apiClient.post(`/api/printer`, data);
      }
    },
    onSuccess: () => {
      toast.success(`Printer ${printer ? "updated" : "created"} successfully`);
      queryClient.invalidateQueries({ queryKey: ["printers"] });
      onClose();
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message || `Failed to ${printer ? "update" : "create"} printer`);
    }
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return toast.error("Printer name is required");
    if (!ip.trim()) return toast.error("IP Address is required");
    if (!siteId || siteId === "all") return toast.error("Valid Site must be selected");

    mutation.mutate({ 
      name: name.trim(), 
      ip: ip.trim(), 
      siteId,
      isDefault,
      printSimultaneously
    });
  };

  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50"
            onClick={onClose}
          />
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 20 }}
            className="fixed left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] p-6"
          >
            <form onSubmit={handleSubmit} className="bg-card border border-border shadow-2xl rounded-3xl overflow-hidden flex flex-col">
              
              <div className="flex items-center justify-between px-8 py-6 border-b border-border/50 bg-secondary/10">
                <h2 className="text-xl font-black uppercase tracking-widest text-foreground">
                  {printer ? "Edit Printer" : "New Printer"}
                </h2>
                <button
                  type="button"
                  onClick={onClose}
                  className="w-8 h-8 rounded-full hover:bg-secondary/80 flex items-center justify-center transition-colors text-muted-foreground"
                >
                  <Icons8 icon="multiply" className="w-4 h-4" />
                </button>
              </div>

              <div className="p-8 flex flex-col gap-6">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-2">Printer Name</label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="e.g. Main Gate Printer"
                    className="w-full h-12 px-4 bg-background border border-border rounded-xl text-[13px] font-bold text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                  />
                </div>

                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-2">IP Address / Hostname</label>
                  <input
                    type="text"
                    value={ip}
                    onChange={(e) => setIp(e.target.value.replace(/ /g, "."))}
                    placeholder="e.g. 192.168.1.100"
                    className="w-full h-12 px-4 bg-background border border-border rounded-xl text-[13px] font-bold text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                  />
                </div>

                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-2">Assigned Facility</label>
                  <select
                    value={siteId}
                    onChange={(e) => setSiteId(e.target.value)}
                    className="w-full h-12 px-4 bg-background border border-border rounded-xl text-[13px] font-bold text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all appearance-none"
                  >
                    <option value="" disabled>Select a facility</option>
                    {sites.filter(s => s.id !== "all").map(site => (
                      <option key={site.id} value={site.id}>{site.name}</option>
                    ))}
                  </select>
                </div>

                <div className="flex flex-col gap-4 mt-2">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input 
                      type="checkbox" 
                      checked={isDefault}
                      onChange={(e) => setIsDefault(e.target.checked)}
                      className="w-5 h-5 rounded border-border text-primary focus:ring-primary"
                    />
                    <span className="text-[12px] font-bold text-foreground">Set as Default Printer</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input 
                      type="checkbox" 
                      checked={printSimultaneously}
                      onChange={(e) => setPrintSimultaneously(e.target.checked)}
                      className="w-5 h-5 rounded border-border text-primary focus:ring-primary"
                    />
                    <span className="text-[12px] font-bold text-foreground">Print Simultaneously (with others)</span>
                  </label>
                </div>
              </div>

              <div className="p-6 border-t border-border/50 bg-secondary/10 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-6 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest bg-secondary text-muted-foreground hover:bg-secondary/80 hover:text-foreground transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={mutation.isPending}
                  className="px-6 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest bg-primary text-white hover:bg-primary/90 transition-colors disabled:opacity-50"
                >
                  {mutation.isPending ? "Saving..." : "Save Printer"}
                </button>
              </div>

            </form>
          </motion.div>
        </>
      )}
    </AnimatePresence>
  );
}
